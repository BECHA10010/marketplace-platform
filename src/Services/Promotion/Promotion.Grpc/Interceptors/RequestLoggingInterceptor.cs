...
public class RequestLoggingInterceptor : Interceptor
{
    ...
    public RequestLoggingInterceptor(ILogger<RequestLoggingInterceptor> logger)
    {
        _logger = logger;
    }
        ...
        UnaryServerMethod<TRequest, TResponse> continuation)
    {
        ...
        {
            _logger.LogInformation("Начало обработки gRPC запроса {Method}",
                context.Method);
            ...
            try
            {
                ...
                using (LogContext.PushProperty("StatusCode", context.Status.StatusCode.ToString()))
                {
                    ...
                    _logger.LogInformation("gRPC запрос завершен за {ResponseTime}ms со статусом {StatusCode}",
            ...
            catch (Exception ex)
            {
                ...
                using (LogContext.PushProperty("StatusCode", "Internal"))
                {
                    ...
                    _logger.LogError(ex, "Ошибка при обработке gRPC запроса за {ResponseTime}ms",
